...
export function ReservationGrid({ reservations, loading }: { reservations: Reserva[]; loading: boolean }) {
  ...
  return <div className="table-wrap"><table><thead><tr><th>Reserva</th><th>Cliente</th><th>Vehículo</th><th>Inicio</th><th>Fin</th><th>Importe</th><th>Estado</th></tr></thead><tbody>{reservations.map((reservation) => <tr key={reservation.id}><td className="strong">#{reservation.id}</td><td>Cliente #{reservation.idCliente}</td><td>Vehículo #{reservation.idVehiculo}</td><td>{formatDate(reservation.horaInicio)}</td><td>{formatDate(reservation.horaFin)}</td><td className="strong">${Number(reservation.importeTotal).toLocaleString('es-AR')}</td><td><span className={`status status-${reservation.estadoReserva.toLowerCase()}`}>{reservation.estadoReserva}</span></td></tr>)}</tbody></table></div>
}
